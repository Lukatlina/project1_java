package activity.job;

import character.IndividualTrainee;
import java.util.Random;

public class PromoterJob extends Job {

    private final int charm;

    public PromoterJob(int health, int 일당, int charm) {
        super(health, 일당);
        this.charm = charm;
    }

    @Override
    public void 능력치습득(IndividualTrainee 플레이어){
        Random random = new Random();
        int random값 = random.nextInt(3);
        if (random값 == 0){
            플레이어.addMoney(get일당()+7000);
            플레이어.addCharm(charm+4);
            플레이어.setHealth(플레이어.getHealth() - (getHealth()+5));
            System.out.println("일당 : " + (get일당()+7000) + "/ health : -" + (getHealth()+5) + "/ charm : " + (charm+4));
            System.out.println("집중해서 아르바이트를 끝냈습니다. 일을 너무 잘해서 보너스를 더 받았습니다.");
        } else if (random값 == 1) {
            플레이어.addMoney(get일당()+2000);
            플레이어.addCharm(charm+2);
            플레이어.setHealth(플레이어.getHealth() - (getHealth()+2));
            System.out.println("일당 : " + (get일당()+2000) + "/ health : -" + (getHealth()+2) + "/ charm : " + (charm+2));
            System.out.println("평범하게 아르바이트를 끝냈습니다.");
        }else{
            플레이어.addMoney(get일당());
            플레이어.addCharm(charm);
            플레이어.setHealth(플레이어.getHealth() - getHealth());
            System.out.println("일당 : " + get일당() + "/ health : -" + getHealth() + "/ charm : " + charm);
            System.out.println("실수를 많이 했습니다. 실수로 인한 피해로 일당이 깎였습니다.");
        }
    }
}
